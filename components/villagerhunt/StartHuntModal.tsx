        ...
        <Autocomplete
          ...
          renderOption={(props, option) => (
            <Box component="li" {...props} key={option.villager_id} sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Avatar src={`/villagers/${option.name.replace(/[^a-zA-Z0-9\u00C0-\u017F]/g, '_')}.png`} alt={option.name} sx={{ width: 24, height: 24 }} />
              {option.name}
            </Box>
          )}
          ...
          renderTags={(tagValue) =>
            tagValue.map((option) => (
              <Box key={option.villager_id} sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                <Avatar src={`/villagers/${option.name.replace(/[^a-zA-Z0-9\u00C0-\u017F]/g, '_')}.png`} alt={option.name} sx={{ width: 20, height: 20 }} />
                {option.name}
              </Box>
            ))
          }
