...
export async function POST(request: NextRequest) {
  ...
  // Update hunt to COMPLETED
  const { error: updateError } = await supabase
    .from('hunts')
    .update({ hunt_status: 'COMPLETED', complete_ts: new Date().toISOString() })
    .eq('hunt_id', huntId);

  if (updateError) {
    console.error('Failed to complete hunt:', updateError);
    return NextResponse.json({ error: 'Failed to complete hunt' }, { status: 500 });
  }

  // Broadcast hunt complete to WebSocket server
  try {
    await fetch('https://villagerhunt-websocket.fly.dev/broadcast', {
      ...
      body: JSON.stringify({
        ...
        payload: { action: 'hunt_completed', hunt_id: huntId },
      }),
    });
  } catch (broadcastError) {
    console.error('Failed to broadcast hunt complete:', broadcastError);
